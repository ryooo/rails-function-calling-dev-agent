    ...
    class Programmer < Base
      def work(reviewer_comment: nil)
        ...
        message_container.add_system_message("You are an excellent Ruby programmer. \n" + \
          ...
          "When implementing the test code, check the rspec results and modify them one by one to pass the tests.\n" + \
      ...
      def make_pr!
        generate_pr_info_function = AzureOpenAi::Functions::GeneratePrInfo.new
        azure_open_ai = AzureOpenAi::Client.new
        io, _, _ = azure_open_ai.chat_with_function_calling_loop(
          messages: [
            {
              ...
              content: "You are an excellent Ruby programmer. \n" + \
                "Please call generate appropriate pull request information function for the following diff.\n\n #{self.get_current_diff}",
            }
          ],
          functions: [generate_pr_info_function],
          color: @color,
          actor_name: self.actor_name,
        )
        ...
        exec_sh("gh pr create --base main --head #{generate_pr_info_function.branch_name} " + \
          "--title '#{generate_pr_info_function.title}' --body '#{generate_pr_info_function.description}'")
      end
    end
  end
end